...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # messages.success(request, 'Go to your email to activate your account')
...
def activate(request,uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'You verified your account succesfully')
        return redirect('login')  
    else: 
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            send_email = EmailMessage(mail_subject, message, to=[to_email])
            send_email.send()
            messages.success(request, 'Email with link to reset your password has been sent to your email address')
            return redirect('login')
